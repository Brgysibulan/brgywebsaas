...
const form = document.querySelector('#superadmin-login-form');
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (!profile) throw new Error('No Super Admin profile is assigned to this account.');
    if (String(profile.role || '').toLowerCase() !== 'super_admin') throw new Error('This login is for Super Admin accounts only.');
    ...
    show('Signed in. Opening Super Admin dashboard…');
