...
type Barangay = { id: number; name: string; municipality: string; province: string; status: "Active" | "Inactive" };
...
export default function SuperAdminPage() {
  ...
  function reset() { setForm({ name: "", municipality: "", province: "" }); setEditingId(null); }
  function submit(event: FormEvent) {
    event.preventDefault();
    const name = form.name.trim();
    if (!name) return;
    if (editingId) {
      setBarangays((items) => items.map((item) => item.id === editingId ? { ...item, ...form, name } : item));
      setMessage(`${name} updated successfully.`);
    } else {
      setBarangays((items) => [...items, { id: Date.now(), ...form, name, status: "Active" }]);
      setMessage(`${name} added successfully.`);
    }
    reset();
  }
  function edit(item: Barangay) { setEditingId(item.id); setForm({ name: item.name, municipality: item.municipality, province: item.province }); setMessage(""); }
  function toggleStatus(id: number) { setBarangays((items) => items.map((item) => item.id === id ? { ...item, status: item.status === "Active" ? "Inactive" : "Active" } : item)); }
